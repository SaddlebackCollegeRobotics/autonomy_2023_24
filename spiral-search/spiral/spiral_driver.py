...
class SpiralDriver(Node):
    ...
    def __init__(self):
        ...
        for _ in range(SpiralDriver.LOOP_RATE):
            self.send_rotate_msg(self.ROTATION_SPEED)
            loop_rate.sleep()

        # Keep spiraling forever, keep track of current iteration to make spiral larger
        #TODO: Change this sometime
        INITIAL_ROT_SPEED: float = 3
    ...
    def send_rotate_msg(self, speed: float):
        ...
        """
        self._twist_msg.linear = Vector3(x=0, y=0, z=0)
        self._twist_msg.angular = Vector3(x=0, y=0, z=speed)
    
    def send_spiral_msg(self, rotation_speed: float, linear_speed: float = 1.0):
        ...
        """
        self._twist_msg.linear = Vector3(x=linear_speed, y=0, z=0)
        self._twist_msg.angular = Vector3(x=0, y=0, z=rotation_speed)
